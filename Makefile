...
test-backend-integration:  ## Backend integration tests
	# Ryuk needs the default bridge network, which is not reachable on
	# every local Docker context. TestMain terminates its own container.
	cd backend && TESTCONTAINERS_RYUK_DISABLED=true \
